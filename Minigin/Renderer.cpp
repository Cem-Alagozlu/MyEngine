...
void Renderer::RenderTexture(SDL_Texture* texture, Rectf srcRect, Vector2f pos, Vector2f scale, double angle, SDL_RendererFlip flipTexture) const
{
	SDL_Rect dst;
	...
	dst.w = int(dst.w * scale.x);
	dst.h = int(dst.h * scale.y);
	SDL_RenderCopyEx(GetSDLRenderer(), texture, &sdlSrcRect, &dst, angle, nullptr, flipTexture);
}